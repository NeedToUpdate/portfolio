...
The difference between that outcome and a vague prompt is the difference between a spec and a wish. "Make a nice space background" produces a stock particle cloud. I never specified an opacity value myself. I said the gas should look translucent and named the order it should stack in: a dense star field first, gaseous point clouds above it, a layer of wisps above that, young stars scattered on top of all of it. The ChatGPT conversation that followed turned "translucent" into an opacity band for every layer. "Use good CSS practices" produces nothing checkable. "Every flex child gets `min-width: 0`, every image keeps `object-fit`, no fixed heights" produces a page that survives a phone with a collapsing URL bar.

<Note>
  The plan is also the review standard. When the first nebula render looked like
  a rainbow smear, the fix was one line of feedback: match the opacity budget in
  the document. Without the document, that conversation takes ten rounds of
