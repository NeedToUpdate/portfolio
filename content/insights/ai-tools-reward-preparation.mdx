...
The difference between that outcome and a vague prompt is the difference between a spec and a wish. "Make a nice space background" produces a stock particle cloud. "A dense star field as a layer, then gaseous point clouds on top of it, then a layer of wisps, then young stars on top of all of it" produces the thing you actually pictured. "Use good CSS practices" produces nothing checkable. "Go find the rules on [defensiveCSS.dev](https://defensivecss.dev/) and put them in the document" produces a page that survives a phone with a collapsing URL bar. Neither instruction named a number. Both named a structure or a source, and the model filled in the specifics from there: the opacity band for every layer, the checklist behind `min-width: 0` and `object-fit`.
