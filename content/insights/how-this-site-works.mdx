...
CloudFront sits in front with two origins. Requests for static assets go to an S3 bucket with immutable cache headers. Page requests and API calls go to the Lambda. Next.js routes each request inside the same application, so an API endpoint needs no separate gateway or service. The incremental cache lives in S3 because a Lambda's filesystem is read-only and two instances share no memory.
