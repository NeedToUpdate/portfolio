...
<FloatImage
  src="/images/sdlc-shared-spine.svg"
  alt="Product, Development, QA, and DevOps form a lifecycle around a connected ticket, code, and build. Design, approvals, test results, release, and rollback records attach to that shared core, keeping business intent connected to implementation and delivery."
  width={1000}
  height={1080}
  side="none"
  caption="Every team works from the same connected ticket, code, and build. Delivery feeds back into product decisions; rollback refers to a previously retained build."
/>
...
- `dev` is where upcoming work integrates
- `feat/$ticket` carries one ticket
- `release/$release_ticket` carries one release

Four rules:

1. Always branch from `main`.
2. `dev` never merges into `main`.
3. Every branch and build is tied to an ID, a ticket or a build number.
4. After a release ships, reconcile `main` and retrofit the releases still in flight.

</Card>

Notice there are no `hotfix/`, `refactor/`, or `fix/` branches for getting something out quickly. A `feat/` branch is already the fastest path available: branch from `main`, merge into a release, build, validate, ship. The simplest one-line fix and a complex multi-team project use the same process at their own speeds.
